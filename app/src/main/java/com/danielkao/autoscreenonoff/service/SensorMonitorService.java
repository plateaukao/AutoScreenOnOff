...
public class SensorMonitorService extends Service implements
    ...
    // notification ids and pending-intent request codes
    private static final int NOTIFICATION_ONGOING = 12345;
    private static final int NOTIFICATION_ADMIN_PROMPT = 12346;
    private static final int RC_NOTI_APP = 10;
    private static final int RC_NOTI_TOGGLE = 11;
    private static final int RC_NOTI_SCREENOFF = 12;
    private static final int RC_ADMIN_PROMPT = 13;
    ...
    /**
     * The service exists while any of these is on; power connect/disconnect
     * is observed from inside (manifest receivers stopped getting those
     * broadcasts in Android 8).
     */
    private boolean shouldRun() {
        return CV.getPrefAutoOnoff(this)
                || CV.getPrefChargingOn(this)
                || CV.getPrefShowNotification(this);
    }
    ...
    private final BroadcastReceiver chargingReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_POWER_CONNECTED.equals(action)) {
                ...
                if (isRegistered() && !CV.getPrefAutoOnoff(context))
                    unregisterSensor();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // immediately, before any branching.
        startAsForeground();
        ...
        if (intent == null) {
            ...
            // 1. autoOn is on (and not inside the sleep window)
        ...
        switch(action){
            ...
            {
                // The ongoing notification is mandatory while monitoring (it
                // backs the foreground service); this action only matters when
            ...
            case CV.SERVICEACTION_TURNON:
            {
                ...
                // from the charging path: only register when actually plugged
                // (or when auto mode is on anyway)
                if(!isRegistered()
                        && (CV.getPrefAutoOnoff(this) || CV.isPlugged(this))){
                    registerSensor();
    ...
    /**
     * central lifecycle decision: when nothing needs the service anymore,
     * drop out of the foreground and stop.
     */
    private int stopIfIdle() {
        if (shouldRun())
            return START_STICKY;
	...
	public void onCreate() {
        ...
        // charging events are only visible to runtime receivers since O
        IntentFilter filter = new IntentFilter();
        filter.addAction(Intent.ACTION_POWER_CONNECTED);
        filter.addAction(Intent.ACTION_POWER_DISCONNECTED);
        ContextCompat.registerReceiver(this, chargingReceiver, filter,
	...
	public void registerSensor() {
		...
		// need device management privilege to lock the screen
		if(!isActiveAdmin()){
			promptDeviceAdmin(false);
		} else {
            NotificationManagerCompat.from(this).cancel(NOTIFICATION_ADMIN_PROMPT);
        }
        ...
        if(type == Sensor.TYPE_PROXIMITY){
            ...
            if (isActiveAdmin()) {
                ...
                if(2 == CV.getPrefTimeoutLock(this)){
                    ...
                    if(mPowerManager.isInteractive() && swipeCount >=6){
                        resetSwipeCount();
                        turnOff();
                        return;
                    }
    ...
    /**
     * permission-free in-call detection (covers telephony and VoIP); the old
     * PhoneStateListener needs READ_PHONE_STATE at runtime since Android 12.
     */
    ...
    /**
     * create notification backing the foreground service; also used to
     * refresh the toggle state shown in it
     */
    private android.app.Notification createNotification(){
        Intent intentOnOff = CV.serviceIntent(this, CV.SERVICEACTION_TOGGLE);
        intentOnOff.putExtra(CV.SERVICETYPE, CV.SERVICETYPE_NOTIFICATION);
        PendingIntent piOnOff = CV.servicePendingIntent(this, RC_NOTI_TOGGLE, intentOnOff);

        boolean bStatusOn = CV.getPrefAutoOnoff(this)
        ...
        PendingIntent piApp = PendingIntent.getActivity(this, RC_NOTI_APP, intentApp,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        Intent intentScreenOff = CV.serviceIntent(this, CV.SERVICEACTION_SCREENOFF);
        PendingIntent piScreenOff = CV.servicePendingIntent(this, RC_NOTI_SCREENOFF, intentScreenOff);
    ...
    private void startAsForeground(){
        int type = (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE)
                ...
                : 0;
        ServiceCompat.startForeground(this, NOTIFICATION_ONGOING, createNotification(), type);
    }
    ...
    /**
     * device admin isn't granted yet: try to open the grant screen (works when
     * the app is visible) and leave a notification as the fallback path —
     * activity starts from the background are blocked since Android 10.
     */
    private void promptDeviceAdmin(boolean closeAfter) {
        Intent i = new Intent(this, MainActivity.class);
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        i.putExtra(CV.CLOSE_AFTER, closeAfter);
        try {
            startActivity(i);
        } catch (Exception e) {
            CV.logi("could not start device-admin activity");
        }

        PendingIntent pi = PendingIntent.getActivity(this, RC_ADMIN_PROMPT, i,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        android.app.Notification n = new NotificationCompat.Builder(this, CHANNEL_ID)
                .setSmallIcon(R.drawable.statusbar_off)
                .setContentTitle(getString(R.string.app_name))
                .setContentText(getString(R.string.device_management_explanation))
                .setContentIntent(pi)
                .setAutoCancel(true)
                .build();
        NotificationManagerCompat nm = NotificationManagerCompat.from(this);
        if (nm.areNotificationsEnabled())
            nm.notify(NOTIFICATION_ADMIN_PROMPT, n);
    }
    //</editor-fold>
    ...
    private void setSchedule() {
        ...
        PendingIntent pi = CV.servicePendingIntent(this, 0, intent);
        ...
        pi = CV.servicePendingIntent(this, 0, intent);
