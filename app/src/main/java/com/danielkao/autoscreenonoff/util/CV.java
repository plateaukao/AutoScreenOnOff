...
public final class CV {
    ...
    /**
     * Explicit intent for SensorMonitorService — implicit service intents
     * are rejected since API 21.
     */
    public static Intent serviceIntent(Context context, int action) {
        Intent i = new Intent(context, SensorMonitorService.class);
        i.putExtra(SERVICEACTION, action);
        return i;
    }

    /**
     * Start SensorMonitorService from any context. The service always
     * promotes itself to foreground, so use the FGS entry point on O+
     * (plain startService throws IllegalStateException from the background).
     */
    ...
    /**
     * PendingIntent that starts SensorMonitorService as a foreground service.
     * Callers must use distinct request codes: the intents differ only in
     * extras, which PendingIntent ignores when matching.
     */
    public static PendingIntent servicePendingIntent(Context context, int requestCode, Intent intent) {
    ...
    public static boolean isPlugged(Context context){
        ...
        return intentBat != null && (intentBat.getIntExtra(BatteryManager.EXTRA_PLUGGED, -1) > 0);
    }
